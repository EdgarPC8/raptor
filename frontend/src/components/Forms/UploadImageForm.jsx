/** Formulario subir imagen al servidor /img. */
import { useEffect, useState } from "react";
import { Box, Button, Checkbox, FormControlLabel, Stack, TextField, Typography } from "@mui/material";
import { uploadImageRequest } from "../../api/imgRequest.js";
import { useAuth } from "../../context/AuthContext.jsx";

export default function UploadImageForm({ onClose, defaultFolder = "", onUploaded }) {
  const { toast } = useAuth();
  ...
  const submit = async () => {
    if (!file) {
      toast({ message: "Selecciona una imagen", variant: "warning" });
      return;
    }
    try {
      await toast({
        promise: uploadImageRequest({ file, folder: folder.trim(), name: name.trim(), replace }),
      });
      setFile(null);
      setName("");
      onUploaded?.();
